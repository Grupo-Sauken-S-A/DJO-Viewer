...
- **Las 6 secciones repetibles muestran su cantidad solo como badge de conteo en el título** (`Agreement`, `GoodVariant`, `LocalSupplies`, `IntrazoneSupplies`, `ExtrazoneSupplies`, `PACThirdCountrySupplies`) — **no** como `Field`/`addField` explícito adicional. Hubo un experimento el 2026-09-04 (v1.4.1) que agregaba ese campo explícito para las 5 no-`Agreement`, pensando que el badge mostraba un conteo real independiente del valor declarado — resultó ser el mismo dato duplicado (el usuario lo vio en producción y lo hizo revertir, v1.4.3). No reintroducir ese campo sin que el usuario lo pida explícitamente; ver el detalle de por qué el badge no es uniforme entre secciones en [`BUSINESS_RULES.md` §9](docs/BUSINESS_RULES.md#9-estructura-del-xml-real-vs-jerarquía-visual).
