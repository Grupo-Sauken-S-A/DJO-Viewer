...
**Las 6 secciones repetibles (`Agreement`, `GoodVariant`, `LocalSupplies`, `IntrazoneSupplies`, `ExtrazoneSupplies`, `PACThirdCountrySupplies`) muestran su cantidad únicamente como badge de conteo en el título de la sección** (`count` de `Section`/`addSection`), sin un `Field`/`addField` explícito adicional para el `*Qty` (confirmado 2026-09-04, revirtiendo un experimento de la misma fecha que sí lo agregaba — ver CHANGELOG v1.4.1/v1.4.3). **Ojo con la fuente de ese badge, que no es uniforme**: para `Agreement` (solo en v2.0.0) es el conteo real de elementos `<Agreement>` encontrados en el XML (`agreements.length`); para las otras 5, es directamente el valor declarado en su tag `*Qty` (`parseInt(...Qty.textContent)`), no un conteo independiente de `<...Item>` presentes — así que agregarles un `Field` con ese mismo valor era pura duplicación visual, no una segunda fuente de verdad (a diferencia de `Agreement`, donde sí lo era, pero el dueño del proyecto decidió que ni ahí hace falta mostrar las dos). Si se agrega una sección repetible nueva, no asumir cuál de los dos criterios de conteo aplica sin revisar el código — y no reintroducir el `Field`/`addField` duplicado sin que el usuario lo pida explícitamente.
